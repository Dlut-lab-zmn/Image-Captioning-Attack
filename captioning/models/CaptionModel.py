...
class CaptionModel(nn.Module):
    ...
    def beam_search(self, init_state, init_logprobs, *args, **kwargs):
        ...
        def beam_step(logprobs, unaug_logprobs, beam_size, t, beam_seq, beam_seq_logprobs, beam_logprobs_sum, state):
            ...
            #print("beam_seq_logprobshape:",beam_seq_logprobs.shape)
            assert (_tmp_beam_logprobs == beam_logprobs).all()
            beam_seq_logprobs = torch.cat([
                beam_seq_logprobs,
                beam_logprobs.reshape(batch_size, -1, 1, vocab_size)], 2)
            #print("beam_seq_logprobsshape:",beam_seq_logprobs.shape)
            new_state = [None for _ in state]
            #print("state_ix:",state_ix)
            #print("stateshape11111111111111111:",state[0].detach().cpu().numpy().shape)
            for _ix in range(len(new_state)):
                #  copy over state in previous beam q to new beam at vix
                new_state[_ix] = state[_ix][:, state_ix]
            #print("state:",state)
            #print("stateshape:",new_state[0].detach().cpu().numpy().shape)
        ...
        state_table = [[_ for _ in init_state] for _ in range(group_size)]
        logprobs_table = [init_logprobs for _ in range(group_size)]
        """------------------------------------------------------------"""
        # END INIT
        ...
        for t in range(self.seq_length + group_size - 1):#self.seq_length + group_size - 1
            for divm in range(group_size):
                if t >= divm and t <= self.seq_length + divm - 1:
                    # add diversity
                    logprobs = logprobs_table[divm]
                    #print("logprobs1",logprobs)
        ...
        done_beams_table = [[sorted(done_beams_table[b][i], key=lambda x: -x['p'])[:bdash] for i in range(group_size)]
                            for b in range(batch_size)]
        #print("done_beams_table2:",done_beams_table)
        done_beams = [sum(_, []) for _ in done_beams_table]
        #print("done_beams:",done_beams)
        return done_beams

    def insertind_beam_search(self, init_state, init_logprobs,index,value, *args, **kwargs):
            #print("insertind_beam_search")
            # function computes the similarity score to be augmented
            def add_diversity(beam_seq_table, logprobs, t, divm, diversity_lambda, bdash):
                local_time = t - divm
                unaug_logprobs = logprobs.clone()
            ...
            def beam_step(logprobs, unaug_logprobs, beam_size, t, beam_seq, beam_seq_logprobs, beam_logprobs_sum,
                          state):
                # INPUTS:
                # logprobs: probabilities augmented after diversity N*bxV
                # beam_size: obvious
            ...
            opt = kwargs['opt']
            temperature = opt.get('temperature', 1)  # This should not affect beam search, but will affect dbs
            beam_size = 1  # opt.get('beam_size', 10)
            group_size = opt.get('group_size', 1)
            diversity_lambda = opt.get('diversity_lambda', 0.5)
            decoding_constraint = opt.get('decoding_constraint', 0)
            remove_bad_endings = opt.get('remove_bad_endings', 0)
            ...
            for t in range(self.seq_length + group_size - 1):  # self.seq_length + group_size - 1
                ...
                for divm in range(group_size):
                    if t >= divm and t <= self.seq_length + divm - 1:
                        ...
                        # the unaugmented ones for sorting the candidates in the end. # for historical
                        # reasons :-)
                        logprobs, unaug_logprobs = add_diversity(beam_seq_table, logprobs, t, divm, diversity_lambda,
                                                                 bdash)
                        # print("logprobs2",logprobs)
                        # infer new beams
                        beam_seq_table[divm], \
        ...
        #return beam_seq_table[0].squeeze(0),beam_seq_logprobs_table[0].squeeze(0)
